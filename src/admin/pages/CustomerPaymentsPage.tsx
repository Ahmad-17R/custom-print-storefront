...
interface CustomerPayment {
  id: string; customerId: string; invoiceId: string | null; amount: string; method: string
...
function Modal({ item, customers, onClose, onSaved }: { item: Partial<CustomerPayment> | null; customers: Customer[]; onClose: () => void; onSaved: () => void }) {
  const [customerId, setCust] = useState(item?.customerId ?? customers[0]?.id ?? '')
  ...
  const save = async () => {
    ...
    try {
      if (item?.id) await api.patch(`/customer-payments/${item.id}`, { reference: reference || null, notes: notes || null })
      else          await api.post('/customer-payments', { customerId, amount: Number(amount), method, paidDate, reference: reference || null, notes: notes || null })
      onSaved(); onClose()
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#fff', borderRadius: 16, width: 460, padding: 28, fontFamily: FONT }}>
        <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 20, color: DARK }}>{item?.id ? 'Edit Payment' : 'Record Customer Payment'}</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20 }}>
          {!item?.id && <>
            <label>{lbl('Customer *')}<select style={inp} value={customerId} onChange={e => setCust(e.target.value)}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <label>{lbl('Reference #')}<input style={inp} value={reference} onChange={e => setRef(e.target.value)} placeholder="e.g. CHQ-001" /></label>
            </div>
          </>}
...
const COLS = [
  { key: 'customer', label: 'Customer', width: 180 }, { key: 'amount', label: 'Amount', width: 130 },
...
export function CustomerPaymentsPage() {
  ...
  const [modal, setModal]         = useState<Partial<CustomerPayment> | null>(null)

  const load = async () => {
    setLoading(true)
    try {
      const [r, c] = await Promise.all([
        api.get<{ data: CustomerPayment[] }>('/customer-payments?pageSize=200'),
        api.get<{ data: Customer[] }>('/customers?pageSize=500'),
      ])
      setData(r.data); setCustomers(c.data)
  ...
  return (
    <AdminLayout title="Customer Payments" actions={<><SearchInput value={q} onChange={setQ} placeholder="Search customer, reference…" /><Btn label="+ Record Payment" onClick={() => setModal({})} /></>}>
      <StatGrid>
        <StatCard label="Total Payments" value={String(data.length)} sub="All payments" />
        <StatCard label="This Month" value={`AED ${totalThisMonth.toLocaleString()}`} sub="Received this month" color="#10B981" />
        <StatCard label="Total Received" value={`AED ${data.reduce((s, r) => s + Number(r.amount), 0).toLocaleString()}`} sub="All time" color="#1D4ED8" />
        <StatCard label="Avg Payment" value={data.length ? `AED ${(data.reduce((s, r) => s + Number(r.amount), 0) / data.length).toFixed(0)}` : 'AED 0'} sub="Average amount" color="#8B5CF6" />
      </StatGrid>
      {loading ? <div style={{ padding: 40, textAlign: 'center', color: '#94A3B8', fontFamily: FONT }}>Loading…</div>
        : <Table columns={COLS} rows={rows} />}
      {modal !== null && <Modal item={modal} customers={customers} onClose={() => setModal(null)} onSaved={load} />}
    </AdminLayout>
  )
}
