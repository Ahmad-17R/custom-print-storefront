...
function HSlider({ children, itemWidth = 200 }: { children: React.ReactNode; itemWidth?: number }) {
...
export function HomePage() {
  ...
  return (
    <div style={{ width: '100%', background: '#fff', fontFamily: font }}>
      ...
      {/* ── Announcement bar ───────────────────────────────────────────────── */}
      {!annDismissed && (
        <div style={{ background: DARK, color: '#fff', fontSize: 12, padding: '8px 24px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, position: 'relative' }}>
          <span>You are viewing the UAE website. Quality printing delivered across all seven Emirates.</span>
          <span style={{ position: 'absolute', right: 24, cursor: 'pointer', fontSize: 16 }} onClick={() => setAnnDismissed(true)}>✕</span>
        </div>
      )}

      {/* ── Utility sub-nav ────────────────────────────────────────────────── */}
      <div style={{ borderBottom: '1px solid #eee', padding: '6px 24px', display: 'flex', gap: 24, fontSize: 12, color: '#555', flexWrap: 'wrap' }}>
      ...
      {/* ── Main header ────────────────────────────────────────────────────── */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 24, padding: '14px 44px', flexWrap: 'wrap' }}>
        {/* Logo */}
        <Link to="/" style={{ textDecoration: 'none', flexShrink: 0 }}>
          <div style={{ fontSize: 22, fontWeight: 800, letterSpacing: '-0.5px', fontFamily: font }}>
        ...
        <div style={{ flex: 1, display: 'flex', minWidth: 200 }}>
          <input
            ...
            style={{ flex: 1, border: '1.5px solid #ccc', borderRight: 'none', padding: '10px 14px', borderRadius: '4px 0 0 4px', fontSize: 14, fontFamily: font, outline: 'none' }}
          />
          <button
            onClick={() => navigate('/catalog')}
            style={{ background: DARK, color: '#fff', border: 'none', borderRadius: '0 4px 4px 0', padding: '0 18px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
        ...
        {/* Right actions */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 20, fontSize: 12, whiteSpace: 'nowrap', flexWrap: 'wrap' }}>
          ...
          {user ? (
            ...
            <button
              ...
              style={{ background: BLUE, color: '#fff', border: 'none', padding: '8px 16px', borderRadius: 12, fontSize: 13, display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontFamily: font, fontWeight: 600 }}>
      ...
      {/* ── Category nav bar ───────────────────────────────────────────────── */}
      <div style={{ padding: '0 44px 12px', display: 'flex', gap: 20, fontSize: 13, borderBottom: '1px solid #eee', overflowX: 'auto', whiteSpace: 'nowrap' }}>
        {NAV_LINKS.map(link => (
      ...
      {/* ── Promo banner ───────────────────────────────────────────────────── */}
      <div style={{ background: DARK, color: '#fff', padding: '10px 24px', fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <strong>Buy More, Save More: AED 220 off orders AED 730+ | Code: MPW30</strong>
          {' | '}Ends Aug 31 | <Link to="/catalog" style={{ color: '#fff', textDecoration: 'underline' }}>Offer details</Link>
          {' | '}<Link to="/catalog" style={{ color: '#fff', textDecoration: 'underline' }}>Save now</Link>
        </div>
        <span>›</span>
      </div>
      ...
      {/* ── Hero ───────────────────────────────────────────────────────────── */}
      <div style={{ position: 'relative' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 0, height: 380 }}>
          {HERO_IMAGES.map((src, i) => (
            <img key={i} src={src} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
              onError={e => { (e.currentTarget as HTMLImageElement).style.background = '#e2e8f0'; (e.currentTarget as HTMLImageElement).style.display = 'block' }} />
          ))}
        </div>
        {/* Overlay card */}
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', background: DARK, color: '#fff', width: 520, padding: '36px 40px', textAlign: 'center', borderRadius: 12 }}>
          <h1 style={{ fontSize: 26, fontWeight: 700, margin: '0 0 14px', fontFamily: font }}>If you need it, we print it.</h1>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button onClick={() => navigate('/catalog')} style={{ background: '#fff', color: DARK, border: 'none', padding: '10px 20px', fontSize: 13, borderRadius: 12, cursor: 'pointer', fontFamily: font, fontWeight: 600 }}>Shop from AED 35</button>
            <button onClick={() => navigate('/catalog')} style={{ background: 'transparent', color: '#fff', border: '1px solid #fff', padding: '10px 20px', fontSize: 13, borderRadius: 12, cursor: 'pointer', fontFamily: font }}>Bestselling Products</button>
            <button onClick={() => navigate('/editor')} style={{ background: 'transparent', color: '#fff', border: '1px solid #fff', padding: '10px 20px', fontSize: 13, borderRadius: 12, cursor: 'pointer', fontFamily: font }}>Design Studio</button>
          </div>
        </div>
      </div>

      {/* ── Trust bar ──────────────────────────────────────────────────────── */}
      <div style={{ display: 'flex', justifyContent: 'space-around', padding: '20px 44px', borderTop: '1px solid #eee', borderBottom: '1px solid #eee', flexWrap: 'wrap', gap: 16 }}>
        {TRUST_ITEMS.map(t => (
      ...
      <section style={{ padding: '32px 44px 8px' }}>
        <h2 style={{ fontSize: 20, fontWeight: 700, margin: '0 0 16px', fontFamily: font }}>Explore all categories</h2>
        <HSlider itemWidth={180}>
          {CATEGORIES.map(c => (
            <Link key={c.name} to={c.slug} style={{ textDecoration: 'none', color: DARK, flexShrink: 0, width: 180 }}>
              <img src={c.img} alt={c.name} style={{ width: 180, height: 140, objectFit: 'cover', borderRadius: 12, display: 'block', background: '#f2efe8' }}
                onError={e => { (e.currentTarget as HTMLImageElement).style.background = '#e2e8f0' }} />
              <div style={{ fontSize: 13, marginTop: 8, lineHeight: 1.3, fontWeight: 500 }}>{c.name}</div>
            </Link>
          ))}
        </HSlider>
      </section>
      ...
      {/* ── Popular products ───────────────────────────────────────────────── */}
      <section style={{ padding: '32px 44px 8px' }}>
        <h2 style={{ fontSize: 20, fontWeight: 700, margin: '0 0 16px', fontFamily: font }}>Our most popular products</h2>
        <HSlider itemWidth={210}>
          {POPULAR_ITEMS.map(r => (
            <Link key={r.name} to={r.slug} style={{ textDecoration: 'none', color: DARK, flexShrink: 0, width: 210 }}>
              <img src={r.img} alt={r.name} style={{ width: 210, height: 170, objectFit: 'cover', borderRadius: 12, display: 'block', background: '#fafafa' }}
                onError={e => { (e.currentTarget as HTMLImageElement).style.background = '#e2e8f0' }} />
      ...
      <section style={{ padding: '32px 44px 8px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 16 }}>
          <h2 style={{ fontSize: 20, fontWeight: 700, margin: 0, fontFamily: font }}>Shop by collection</h2>
          <Link to="/catalog" style={{ fontSize: 13, color: BLUE }}>See all collections →</Link>
        </div>
        <HSlider itemWidth={210}>
          {COLLECTIONS.map(col => (
            <Link key={col.name} to={col.slug} style={{ textDecoration: 'none', color: DARK, flexShrink: 0, width: 210 }}>
              <img src={col.img} alt={col.name} style={{ width: 210, height: 150, objectFit: 'cover', borderRadius: 12, display: 'block' }}
                onError={e => { (e.currentTarget as HTMLImageElement).style.background = '#e2e8f0' }} />
              <div style={{ fontSize: 13, marginTop: 8, lineHeight: 1.3, fontWeight: 500 }}>{col.name}</div>
            </Link>
          ))}
        </HSlider>
      </section>
      ...
      {/* ── Must-haves ─────────────────────────────────────────────────────── */}
      <section style={{ padding: '32px 44px 8px' }}>
        <h2 style={{ fontSize: 20, fontWeight: 700, margin: '0 0 16px', fontFamily: font }}>Explore even more must-haves</h2>
        <HSlider itemWidth={210}>
          {MUST_HAVES.map(m => (
            <Link key={m.name} to={m.slug} style={{ textDecoration: 'none', color: DARK, flexShrink: 0, width: 210 }}>
              <img src={m.img} alt={m.name} style={{ width: 210, height: 150, objectFit: 'cover', borderRadius: 12, display: 'block', background: '#f2efe8' }}
                onError={e => { (e.currentTarget as HTMLImageElement).style.background = '#e2e8f0' }} />
              <div style={{ fontSize: 13, marginTop: 8, fontWeight: 500 }}>{m.name}</div>
            </Link>
          ))}
        </HSlider>
      </section>
      ...
      {/* ── Promo split card ───────────────────────────────────────────────── */}
      <section style={{ margin: '32px 44px' }}><div style={{ background: LIGHT, borderRadius: 12, display: 'grid', gridTemplateColumns: '1fr 1fr', overflow: 'hidden' }}>
        <div style={{ padding: '48px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 14 }}>
          <h2 style={{ fontSize: 26, fontWeight: 700, margin: 0, fontFamily: font }}>Flyers that mean business, for less.</h2>
        ...
        <img src={u('photo-1561070791-2526d30994b5', 600, 400)} alt="Flyers" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
      ...
      <section style={{ padding: '32px 44px 8px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 16 }}>
          <h2 style={{ fontSize: 20, fontWeight: 700, margin: 0, fontFamily: font }}>Tools to help build your business</h2>
          <Link to="/editor" style={{ fontSize: 13, color: BLUE }}>See all services →</Link>
        </div>
        <HSlider itemWidth={220}>
          {TOOLS.map(tool => (
            <Link key={tool.title} to={tool.slug} style={{ textDecoration: 'none', color: DARK, border: '1px solid #eee', borderRadius: 12, padding: 20, display: 'flex', flexDirection: 'column', gap: 12, flexShrink: 0, width: 220 }}>
              <div style={{ fontSize: 14, fontWeight: 700, fontFamily: font }}>{tool.title}</div>
              <img src={tool.img} alt={tool.name} style={{ width: '100%', height: 110, objectFit: 'cover', borderRadius: 12, display: 'block' }}
                onError={e => { (e.currentTarget as HTMLImageElement).style.background = '#e2e8f0' }} />
              <div style={{ fontSize: 12, color: '#666' }}>{tool.name}</div>
            </Link>
          ))}
        </HSlider>
      </section>
      ...
      {/* ── UGC Gallery ────────────────────────────────────────────────────── */}
      <section style={{ padding: '32px 44px 8px' }}>
        <h2 style={{ fontSize: 20, fontWeight: 700, margin: '0 0 4px', fontFamily: font }}>Made by you, #MadeWithMyPrintingWorld</h2>
        <p style={{ fontSize: 13, color: '#666', margin: '0 0 16px' }}>We love to see your custom creations. Tag us @MyPrintingWorld and #MadeWithMyPrintingWorld for a chance to be featured.</p>
        <HSlider itemWidth={210}>
          {GALLERY.map(g => (
            <div key={g.handle} style={{ flexShrink: 0, width: 210 }}>
              <img src={g.img} alt={g.handle} style={{ width: 210, height: 150, objectFit: 'cover', display: 'block', borderRadius: 12 }}
                onError={e => { (e.currentTarget as HTMLImageElement).style.background = '#e2e8f0' }} />
      ...
      {/* ── About section ──────────────────────────────────────────────────── */}
      <section style={{ background: '#F8FAFC', marginTop: 24 }}><div style={{ padding: '56px 44px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60 }}>
        <div>
          <h3 style={{ fontSize: 18, fontWeight: 700, margin: '0 0 12px', fontFamily: font }}>MyPrintingWorld: Here for UAE business.</h3>
      ...
      {/* ── Footer ─────────────────────────────────────────────────────────── */}
      <footer style={{ background: DARK }}><div style={{ color: '#cdd5e0', padding: '40px 44px', display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1fr', gap: 30, flexWrap: 'wrap' }}>
        <div>
      ...
      {/* ── Footer bottom bar ──────────────────────────────────────────────── */}
      <div style={{ background: '#0d1826' }}><div style={{ color: '#9db4d4', padding: '14px 44px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 11, flexWrap: 'wrap', gap: 10 }}>
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
          <span>+971 4 000 0000</span>
          <Link to="/" style={{ color: '#9db4d4', textDecoration: 'none' }}>Home</Link>
          <a href="#" style={{ color: '#9db4d4', textDecoration: 'none' }}>Privacy & Cookie Policy</a>
          <a href="#" style={{ color: '#9db4d4', textDecoration: 'none' }}>Terms and Conditions</a>
          <a href="#" style={{ color: '#9db4d4', textDecoration: 'none' }}>Legal</a>
        </div>
        <div>© 2024–{year} MyPrintingWorld. All rights reserved.</div>
      </div></div>
